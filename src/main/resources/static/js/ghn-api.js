 ...
 * 
 */
const TOKEN = '104ba9da-1130-11f1-a3d6-dac90fb956b5';
const SHOP_ID = 199387;
$(document).ready(function () {
  ...
  $('#provinceSelect').on('change', function () {
    ...
    $('#districtSelect').prop('disabled', true).html('<option value="">--Select district/county--</option>').trigger('change');
    $('#wardSelect').prop('disabled', true).html('<option value="">--Select commune/ward--</option>').trigger('change');
    if (provinceId) {
...
async function calculateShippingFee(toDistrict, toWard, serviceId, items ) {
  ...
  try {
    const resp = await fetch('https://dev-online-gateway.ghn.vn/shiip/public-api/v2/shipping-order/fee', {
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'Token': TOKEN,
